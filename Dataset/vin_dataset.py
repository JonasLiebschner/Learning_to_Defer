...
class BasicDatasetVinBig(BasicDataset):
    ...
    def __init__(self, Path, target_id):
        df = pd.read_csv(f"{Path}/train.csv").drop(["x_min", "y_min", "x_max", "y_max"], axis=1)
        remove_findings = [target_id, 14] #Allows only the finding and no finding
        df = df[df["class_id"].isin(remove_findings)]

        self.data = self.processDF(df)
        self.data.rename(columns={"image_id": 'Image ID'}, inplace=True)
        self.data = self.buildGT(self.data)

        print("Number of images of the whole dataset: " + str(len(self.data["Image ID"].values)))

    def processDF(self, df):
        reader_dfs = {}
        for reader_id in df["rad_id"].unique():
            reader_dfs[reader_id] = df[df["rad_id"] == reader_id].copy().drop(["class_name", "rad_id"], axis=1).drop_duplicates()
            reader_dfs[reader_id].rename(columns={'class_id':f'{reader_id}'[1:]}, inplace=True)
        
        result_df = reader_dfs[list(reader_dfs.keys())[0]]
        for key in list(reader_dfs.keys())[1:]:
            result_df = pd.merge(result_df, reader_dfs[key], how="outer", left_on='image_id', right_on='image_id')
        return result_df

    def buildGT(self, df):
        df["votes"] = df.apply(lambda x: x.drop(["Image ID"]).count(), axis=1)
        df = df[df["votes"] >= 3].drop(["votes"], axis=1)
        df["GT"] = self.majorityVote(df)
        df["GT"] = df["GT"].apply(lambda x: 1 if x ==0 else 0)
        return df

    def majorityVote(self, df):
        return df.apply(lambda x: x.drop(["Image ID"]).mode()[0], axis=1)
...
class VINDataset(Dataset):
    ...
    def loadImage(self, idx):
        ...
        if self.image_container is not None:
            ...
            return Image.open(self.PATH + "train/" + self.image_ids[idx]).convert("RGB").resize(self.size)
...
class VIN_K_Fold_Dataloader(K_Fold_Dataloader):
    def __init__(self, dataset, k=10, labelerIds=[4323195249 , 4295194124], train_batch_size=8, test_batch_size=8,
        ...
        if self.common:
            ...
            for labelerId in self.labelerIds:
                ...
                temp = self.dataset[self.dataset[str(labelerId)].notnull()]["Image ID"].values.tolist()
    ...
    def _init_k_folds(self, fraction=1.0):
        self.labels = self.data.drop_duplicates(subset=["Image ID"])
        self.labels = self.labels.fillna(-1)
        self.labels = self.labels[["Image ID", "GT"]]
        ...
        for fold_idx in range(len(fold_data_idxs)):
            ...
            print(f"Fold {fold_idx}, len of train {len(expert_train)}, len of val {len(expert_val)}, len of test {len(expert_test)}")
    ...
    def create_Dataloader_for_Fold(self, idx):
        ...
        expert_train_dataset = VINDataset(expert_train, preload=self.preload, preprocess=self.preprocess, param=self.param, image_container=self.image_container)
        expert_val_dataset = VINDataset(expert_val, preload=self.preload, preprocess=self.preprocess, param=self.param, image_container=self.image_container)
        expert_test_dataset = VINDataset(expert_test, preload=self.preload, preprocess=self.preprocess, param=self.param, image_container=self.image_container)
    ...
    def getFullDataloader(self):
        full_dataset = VINDataset(self.labels[["Image ID", "GT"]], preload=self.preload, preprocess=self.preprocess, param=self.param, image_container=self.image_container)
...
class ImageContainerVIN(ImageContainer):
    ...
    def loadImage(self, idx):
        ...
        Load one single image
        """
        return Image.fromarray(self.read_xray(self.PATH + "train/" + self.image_ids[idx])).convert("RGB").resize(self.img_size)
...
class DataManagerVIN(DataManager):
    ...
    def createData(self):
        ...
        for seed in self.seeds:
            ...
            self.kFoldDataloaders[seed] = VIN_K_Fold_Dataloader(
            ...
            self.SSLDatasets[seed] = VINSSLDataset(dataset=self.basicDataset, kFoldDataloader=self.kFoldDataloaders[seed], imageContainer=self.fullImageContainer, 
...
class VINSSLDataset(SSLDataset):
    ...
    def create_Dataloader_for_Fold(self, idx):
        ...
        expert_train_dataset = VINDataset(expert_train, preload=self.preload, preprocess=self.preprocess, param=self.param, image_container=self.imageContainer)
        expert_val_dataset = VINDataset(expert_val, preload=self.preload, preprocess=self.preprocess, param=self.param, image_container=self.imageContainer)
        expert_test_dataset = VINDataset(expert_test, preload=self.preload, preprocess=self.preprocess, param=self.param, image_container=self.imageContainer)
    ...
    def getFullDataloader(self):
        full_dataset = VINDataset(self.labels[["Image ID", "GT"]], preload=self.preload, preprocess=self.preprocess, param=self.param, image_container=self.imageContainer)
        return torch.utils.data.DataLoader(dataset=full_dataset, batch_size=self.train_batch_size, num_workers=self.num_workers, shuffle=True, drop_last=False, pin_memory=True)
