...
class BasicDatasetCIFAR10N(dsc.BasicDataset):
    ...
    def get_cifar10_labels(self, path_labels, path_data):
        self.human_labels = torch.load(f"{path_labels}/CIFAR-10_human.pt")
        self.cifar_trainset = self.load_cifar_10(path_data)

        df = pd.DataFrame({"GT": self.cifar_trainset.targets, "1": self.human_labels["random_label1"], "2": self.human_labels["random_label2"],"3": self.human_labels["random_label3"],})
        #df.index.name = "Image ID"
        df = df.rename_axis('Image ID').reset_index()
